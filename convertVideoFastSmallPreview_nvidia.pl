...
my $TARGET_WIDTH="800";
my $TARGET_EXT=".mp4";
my $SUFFIX="_PREVIEW_NV_CQ_" . $CQ . "_W_" . $TARGET_WIDTH.$TARGET_EXT;
...
for (my $c=0; $c < $argcount; $c++){
	...
	say "fOut:" . $fOut;
	my $cmd1 = qq{ffmpeg -y -i "$currFile" -c:v h264_nvenc -rc vbr -cq "$CQ" -qmin "$CQ" -qmax "$CQ" -pix_fmt yuv420p -vf scale=$TARGET_WIDTH:-1  -c:a aac -b:a 64k -ac 1 "$fOut"};
	say "Command is: ". $cmd1;
	system $cmd1;
	if ( $? == 0 ) {
