...
my $TARGET_EXT=".avi";
my $SUFFIX="_ULTRAFAST_CRF_" . $CRF . "_W_" . $TARGET_WIDTH.$TARGET_EXT;
...
for (my $c=0; $c < $argcount; $c++){
	...
	say "fOut:" . $fOut;
	my $cmd1 = qq{ffmpeg -y -i "$currFile" -c:v libx264 -preset ultrafast -profile:v high -crf $CRF -vf scale=$TARGET_WIDTH:-1 -pix_fmt yuv420p -r 30000/1001  -c:a aac -b:a 256k -ac 2 "$fOut"};
	say "Command is: ". $cmd1;
	system $cmd1;
	if ( $? == 0 ) {
